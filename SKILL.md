...
**見出し + 隣接する非見出しテキストの構造的関連付け（暗黙のセクション境界の見落とし）**

見出し要素を見つけたら、その要素単体だけでなく直前の兄弟要素も確認する。見出しの直前に置かれた非見出し要素（`p`, `span`, `div` 等）は、DOM 上はただの前置きに見えても、実際にはその見出しが導入するセクションの一部（サブタイトル・キッカー・代替タイトル）である場合がある。この場合、見出しより後に続くべき情報が視覚的な都合で先に置かれているだけで、DOM の前後関係だけではその見出しとの関連がプログラム的に伝わらない。

直前の非見出し要素が以下のいずれかに該当する場合、この見落としの可能性を検討する:

- 見出しの文言と意味的に重複・言い換えの関係にある（例: 英語ラベルと日本語見出しが同じ内容を指している）
- スタイル（フォントサイズ・太字・色など）が見出しと呼応しており、視覚的に見出しの一部として設計されている
- 見出し以降に続く本文とは無関係で、見出し自体の補足情報（サブタイトル・キャッチコピー・代替タイトル）としてしか意味をなさない

該当する場合、以下を優先的に検討する:

1. `hgroup` 要素で見出しとまとめる。WHATWG HTML Living Standard § 4.3.10 は `hgroup` を「見出しと関連コンテンツ」を表す要素と定義し、コンテンツモデルを「0個以上の `p` 要素、続けて1つの `h1`〜`h6` 要素、続けて0個以上の `p` 要素」としている。見出しの**前**に置かれる `p` も明示的に許可された用法であり、サブタイトル・代替タイトル・キャッチコピーの例として仕様自身が挙げている。
2. 見出し内で `strong` 等を使い、キッカー部分を見出しテキスト自体に統合する。

一方、直前の非見出し要素が明らかに別の意味単位（前段落の結論文など、その見出しのセクションと無関係な内容）であれば指摘不要。

例:

- `<p class="mb-2 font-bold text-[#DA3734]">Payments</p><h2>お支払い方法</h2>` → 英語ラベルが見出しの言い換えとして機能しており、`hgroup` でまとめるか、いずれか一方に統合するのが妥当
- `<p>（前セクションのまとめ文）</p><h2>次のセクション</h2>` → 意味的に無関係なので指摘不要

> **Claude へ**: 「見出しの前に非見出しテキストがある」というだけで機械的に指摘しないこと。上記の意味的重複・視覚的な見出し帯同のシグナルの有無で判断する。逆に、見出しレベルの飛び番チェックだけに意識を引っ張られて、この隣接関係のチェックを省略しないこと。
...
- 見出し（`h1`〜`h6`）の階層構造が適切か（飛び番がないか）。加えて、見出しに隣接する非見出しテキスト（サブタイトル・キッカー等）が構造的に紐付いているか（4-1「見出し + 隣接する非見出しテキストの構造的関連付け」参照）
